...
class ResearchEngineTests(unittest.TestCase):
    ...
    def test_frozen_model_has_pre_2025_selection_cutoff(self) -> None:
        model = json.loads(
            (WORK_DIR / "july26_model.json").read_text(encoding="utf-8")
        )
        self.assertLess(
    ...
    def test_saved_candidate_beats_identical_panel_rl2_results(self) -> None:
        summary = json.loads(
            (WORK_DIR / "july26_summary.json").read_text(encoding="utf-8")
        )
        rl2 = json.loads(
            (WORK_DIR / "rl2_benchmark.json").read_text(encoding="utf-8")
        )
    ...
    def test_latest_overlay_is_complete(self) -> None:
        manifest = json.loads(
            (WORK_DIR / "refresh_manifest.json").read_text(encoding="utf-8")
        )
