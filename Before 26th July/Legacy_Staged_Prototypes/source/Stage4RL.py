...
df = yf.download("AAPL", start="2018-01-01", end="2024-12-31", multi_level_index=False)
...
env = gym.make(
    ...
    trading_fees=0.000     # 1% execution fee
)

print("🧠 Step 3: Initializing the PPO Brain...")
model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003)

print("🏋️‍♂️ Step 4: Training the AI with 75,000 steps...")
# Bumping it up to 75k steps so it has time to digest the new features
model.learn(total_timesteps=75000)

print("💾 Step 5: Saving the upgraded AI model...")
model.save("ppo_apple_trader_v3")
print("🎉 Success! Your upgraded AI is saved as 'ppo_apple_trader_v3.zip'.")