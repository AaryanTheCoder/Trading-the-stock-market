...
POLICIES_TO_TEST = [
    ("Long-term policy", SCRIPT_DIRECTORY / "rl_codex_2_long_term_policy"),
    ("Original policy", SCRIPT_DIRECTORY / "rl_codex_2_policy"),
]


def trade_log_path(policy_name: str) -> Path:
    safe_name = policy_name.lower().replace(" ", "_").replace("-", "_")
    return SCRIPT_DIRECTORY / f"CODEX_SIMULATION_2_{safe_name.upper()}_TRADES.csv"
