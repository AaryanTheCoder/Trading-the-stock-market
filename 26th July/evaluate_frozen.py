...
def main() -> None:
    model_path = WORK_DIR / "july26_model.json"
    ...
    close_result["trades"].to_csv(WORK_DIR / "july26_close_trades.csv", index=False)
    next_open_result["trades"].to_csv(
        WORK_DIR / "july26_next_open_trades.csv", index=False
    )
    pd.concat(
        {
        ...
        axis=1,
    ).to_csv(WORK_DIR / "july26_equity.csv", index_label="date")
    ...
    stress.to_csv(WORK_DIR / "robustness_checks.csv", index=False)
    ...
    path = WORK_DIR / "july26_summary.json"
